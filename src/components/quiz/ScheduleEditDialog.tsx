...
export default function ScheduleEditDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md sm:rounded-2xl border-slate-100 p-5 md:p-6 bg-white font-['Plus_Jakarta_Sans',sans-serif] max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto pr-1.5 space-y-3.5 py-1">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 h-10 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#0A99AB]/20 focus:border-[#0A99AB] outline-none text-sm text-slate-800 transition-all bg-white"
            />
          </div>
          ...
          {/* Quiz Options */}
          <div className="border-t border-slate-100 pt-3 mt-1.5 space-y-2.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-0.5">
              Quiz Options
            </span>

            {/* Randomize Questions Checkbox */}
            <div className="flex items-start gap-3 p-2.5 bg-slate-50/50 border border-slate-100/50 rounded-xl">
              <input
                type="checkbox"
                id="randomize-questions"
                checked={randomizeQuestionOrder}
                onChange={e => setRandomizeQuestionOrder(e.target.checked)}
                className="w-4.5 h-4.5 rounded border-slate-350 text-[#0A99AB] focus:ring-[#0A99AB]/20 cursor-pointer mt-1 accent-[#0A99AB] flex-shrink-0"
              />
              <div className="space-y-0.5">
                <label className="text-xs font-bold text-slate-850 cursor-pointer select-none" htmlFor="randomize-questions">
                  Randomize Questions
                </label>
            ...
            <div className="flex items-start gap-3 p-2.5 bg-slate-50/50 border border-slate-100/50 rounded-xl">
              ...
              />
              <div className="space-y-0.5">
                <label className="text-xs font-bold text-slate-850 cursor-pointer select-none" htmlFor="shuffle-options">
          ...
          <div className="bg-[#0A99AB]/5 rounded-xl p-2.5 flex items-start gap-2 border border-[#0A99AB]/10 mt-1.5">
            <HelpCircle className="w-3.5 h-3.5 text-[#0A99AB] flex-shrink-0 mt-0.5" />
        ...
        <DialogFooter className="mt-4 border-t border-slate-100 pt-3 flex items-center justify-between sm:justify-end gap-2 flex-shrink-0">
          <button
            type="button"
            onClick={handleClear}
            disabled={!openAt && !closeAt}
            className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold text-red-500 hover:bg-red-50 transition-all mr-auto disabled:opacity-30 disabled:pointer-events-none"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl border border-slate-150 text-xs font-bold text-slate-500 hover:bg-slate-50 transition-all"
          >
            Cancel
          </button>
          ...
          <button
            ...
            className="primary-gradient text-white py-2 px-4 rounded-xl font-bold flex items-center justify-center gap-1.5 shadow-lg shadow-[#0A99AB]/15 hover:scale-[1.01] active:scale-95 transition-all text-xs disabled:opacity-50 disabled:pointer-events-none"
          >
